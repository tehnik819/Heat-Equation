...
public class KrankNikolsonScheme extends Scheme{

    public KrankNikolsonScheme(InitialData initialData) {
        super(initialData);
    }
    ...
    public double[][] calculate() {
        ...
        matrix[1][0] = matrix[Nx - 1][Nx] = -data.a*data.tau/(2.0*data.h*data.h);
        for(int i = 1;i < Nx;i++) {
            matrix[i][i] = 1.0 + data.a*data.tau/(data.h*data.h);
            if(i < Nx) {
                matrix[i][i + 1] = -data.a*data.tau/(2.0*data.h*data.h);
            }
            if(i > 0) {
                matrix[i][i - 1] = -data.a*data.tau/(2.0*data.h*data.h);
            }
        ...
        for(int i = 0;i < Nt;i++) {
            ...
            for(int j = 1;j < Nx;j++) {
                b[j] = data.a*data.tau/(2.0*data.h*data.h)*(result[i][j - 1] + result[i][j + 1])
                       + (1.0 - data.a*data.tau/(data.h*data.h))*result[i][j]
                        //+ data.tau*(data.f(data.x_l + data.h*j, data.t_l + data.tau*(i + 1))); //Как в билете
                        + data.tau*(data.f(data.x_l + data.h*j, data.t_l + data.tau*(i + 1)) + data.f(data.x_l + data.h*j, data.t_l + data.tau*i))/2.0; //Как на самом деле
            }
